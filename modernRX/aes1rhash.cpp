...
namespace modernRX::aes {
    namespace {
		...
		constexpr std::array<std::byte, 64> initial_state = std::bit_cast<std::array<std::byte, 64>>(std::array<uint8_t, 64>{
	...
	void hash1R(std::span<std::byte, 64> output, const std::span<std::byte> input) {
		...
		for (uint64_t i = 0; i < input.size(); i += 64) {
			...
			encode(state[0], stdexp::span_cast<const uint32_t, 4>(offset));
			decode(state[1], stdexp::span_cast<const uint32_t, 4>(offset + 16));
			encode(state[2], stdexp::span_cast<const uint32_t, 4>(offset + 32));
			decode(state[3], stdexp::span_cast<const uint32_t, 4>(offset + 48));
		}
