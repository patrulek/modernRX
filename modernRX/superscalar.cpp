...
namespace modernRX {
	namespace {
		constexpr uint32_t Int_Register_Count{ 8 }; // Number of registers used in CPU simulation.
		...
		using PortsSchedule = std::array<std::array<bool, Rx_Max_Schedule_Cycle>, 8>;
		...
		using RegisterFile = std::array<Register, Int_Register_Count>;

		// Holds information about simulated ASIC latencies for every register and selected address register.
		struct AsicContext {
			std::array<uint32_t, Int_Register_Count> latencies{}; // Estimated latencies for every register.
			uint32_t max_latency{ 0 }; // Max estimated latency over all registers.
			reg_idx_t max_latency_register{ 0 }; // Register index that holds max estimated latency.
		};
		...
		struct ProgramContext {
			...
			uint32_t mul_count{ 0 }; // Number of multiplication instructions.
			uint32_t cycle{ 0 }; // Simulated CPU cycle. Equal to or greater than current decode cycle.
			...
			bool ports_saturated{ false }; // Change to true when no other macro-op could be scheduled.
		};


		std::pair<std::array<ExecutionPort, 2>, uint32_t> scheduleOp(const PortsSchedule& ports, const MacroOp& op, const uint32_t cycle, const uint32_t dependency_cycle);
		std::pair<ExecutionPort, uint32_t> scheduleUop(const PortsSchedule& ports, const ExecutionPort uop_port, const uint32_t cycle);
		void findAvailableRegisters(std::vector<reg_idx_t>& available_registers, const RegisterFile& registers, const uint32_t cycle);
		bool needRegisterDisplacement(const std::vector<reg_idx_t>& available_register);
		void updateAsicContext(AsicContext& asic_ctx, const Instruction& instr);
	}

	Superscalar::Superscalar(blake2b::Random& blakeRNG)
		: blakeRNG(blakeRNG) {}

	void Superscalar::generate(Program& prog) {
		// Number of max tries in case when instruction dont fit in decode buffer.
		constexpr uint32_t Max_Throwaway_Count{ 256 }; 

		// Available registers holds indexes of registers ready at a given CPU cycle.
		std::vector<reg_idx_t> available_registers;
		available_registers.reserve(Int_Register_Count);
		...
		for (ProgramContext ctx{}; !ctx.ports_saturated && ctx.decode_cycle < Rx_Superscalar_Latency && ctx.program_size < Rx_Superscalar_Max_Program_Size; ++ctx.decode_cycle, ++ctx.cycle) {
			...
			// Select instruction for every slot in buffer.
			for (uint32_t decode_buffer_slot = 0; decode_buffer_slot < decode_buffer.size() && decode_buffer[decode_buffer_slot] > 0; ++decode_buffer_slot) {
				...
				if (instruction.issued()) {
					if (ctx.ports_saturated || ctx.program_size >= Rx_Superscalar_Max_Program_Size) {
				...
				if (min_schedule_cycle == Rx_Max_Schedule_Cycle) {
					return; // Cannot schedule instr anymore, ports are saturated.
				}
				auto schedule_cycle = min_schedule_cycle;
				...
				if (op_index == instruction.srcOpIndex() || op_index == instruction.dstOpIndex()) {
					...
					const uint32_t future_schedule_cycle{ std::min(schedule_cycle + Rx_Superscalar_Op_Max_Latency - 1, Rx_Max_Schedule_Cycle - 1) };
					...
					if (available_registers.size() == 0) {
						...
						if (ctx.throwaway_count < Max_Throwaway_Count) {
							...
							// Try another one instruction for this slot.
							continue;
						}
				...
				const auto& [recalculated_schedule_ports, recalculated_schedule_cycle] { scheduleOp(ports, op, schedule_cycle, schedule_cycle) };
				if (recalculated_schedule_cycle == Rx_Max_Schedule_Cycle) {
					return; // Cannot schedule instr anymore, ports are saturated.
				}

				// Update port availability.
				ports[static_cast<uint8_t>(recalculated_schedule_ports[0])][recalculated_schedule_cycle] = true;
				ports[static_cast<uint8_t>(recalculated_schedule_ports[1])][recalculated_schedule_cycle] = true;

				// Calculate when the result will be ready.
				ctx.dependency_cycle = recalculated_schedule_cycle + op.latency;

				// If this instruction writes the result, modify destination register information.
				if (op_index == instruction.info->result_op_index) {
				...
				ctx.cycle = top_cycle;
				ctx.ports_saturated |= (recalculated_schedule_cycle >= Rx_Superscalar_Latency);

				// When all macro-ops of the current instruction have been issued, add the instruction into the program
				if (instruction.issued()) {
					instruction.src_register = instruction.src_register.has_value() ? instruction.src_register : instruction.dst_register;
	...
	Instruction Superscalar::initializeInstruction(const InstructionType type) {
		...
		case InstructionType::ISMULH_R:
			...
			//
			// This seems like an overlook in original implementation that has to stay this way.
	...
	uint8_t Superscalar::selectRegister(const std::vector<reg_idx_t>& available_registers) {
	...
	void executeSuperscalar(std::span<uint64_t, 8> registers, const Program& program) {
		auto signExtend2sCompl = [](const uint32_t x) {
		...
		for (uint32_t i = 0; i < program.size; ++i) {
			...
			case InstructionType::IADD_C9:
				registers[instr.dst_register] += signExtend2sCompl(instr.imm32);
				break;
			case InstructionType::IXOR_C7: [[fallthrough]];
			case InstructionType::IXOR_C8: [[fallthrough]];
			case InstructionType::IXOR_C9:
	...
	namespace {
		// Return up to two ExecutionPort and a cycle for which given macro-op should be scheduled according to rules: https://github.com/tevador/RandomX/blob/master/doc/specs.md#633-port-assignment
		std::pair<std::array<ExecutionPort, 2>, uint32_t> scheduleOp(const PortsSchedule& ports, const MacroOp& op, const uint32_t cycle, const uint32_t dependency_cycle) {
			...
			while (schedule_cycle < Rx_Max_Schedule_Cycle) {
				...
				if (uop1_schedule_cycle != uop2_schedule_cycle || uop1_schedule_cycle == Rx_Max_Schedule_Cycle) {
					++schedule_cycle;
					continue;
				}
			...
			return std::make_pair(std::array<ExecutionPort, 2>{ ExecutionPort::NONE, ExecutionPort::NONE }, Rx_Max_Schedule_Cycle);
		}
		...
		std::pair<ExecutionPort, uint32_t> scheduleUop(const PortsSchedule& ports, const ExecutionPort uop_port, const uint32_t cycle) {
			...
			const auto canScheduleToPort = [&](const ExecutionPort port) {
			...
			return std::make_pair(ExecutionPort::NONE, Rx_Max_Schedule_Cycle);
		}

		// Fills available_registers vector with registers indexes that are ready for given cycle.
		...
		bool needRegisterDisplacement(const std::vector<reg_idx_t>& available_registers) {
		...
		void updateAsicContext(AsicContext& asic_ctx, const Instruction& instr) {
			...
			const uint32_t src_latency{ instr.dst_register != instr.src_register ? asic_ctx.latencies[instr.src_register.value()] + 1 : 0 };
