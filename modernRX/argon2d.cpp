...
namespace modernRX::argon2d {
	namespace {
		...
		std::array<std::byte, Initial_Hash_Size> initialize(const std::span<std::byte> password, const std::span<std::byte> salt, const Params& params);
		void makeFirstPass(Memory& memory, const std::array<std::byte, 64>& hash, const Params& params);
	...
	void fillMemory(Memory& memory, const std::span<std::byte> password, const std::span<std::byte> salt, const Params& params) {
		[[unlikely]] if (salt.size() < 8) {
			throw "invalid salt size";
		}

		[[unlikely]] if (memory.size() != params.memory_blocks) {
	...
	namespace blake2b {
		void gmixing(std::span<uint64_t, 16> v, const uint32_t a, const uint32_t block, const uint32_t block_idx, const uint32_t d) {
		...
		void round(std::span<uint64_t, 16> v) {
			gmixing(v, 0, 4, 8, 12);
			gmixing(v, 1, 5, 9, 13);
			gmixing(v, 2, 6, 10, 14);
			gmixing(v, 3, 7, 11, 15);
		...
		void hash(std::span<std::byte> output, const std::span<std::byte> input) {
			...
			[[unlikely]] if (input.size() == 0) {
				throw "invalid data";
			}

			[[unlikely]] if (output.size() == 0) {
			...
			[[unlikely]] if (digest_size == initial_hash_size) {
				return;
			}

			constexpr uint32_t step = Max_Digest_Size / 2;
			uint32_t to_produce = static_cast<uint32_t>(digest_size - step);
	...
	namespace {
		void validateParams(const Params& params) {
			[[unlikely]] if (params.iterations == 0) {
				throw "invalid # of iterations";
			}

			[[unlikely]] if (params.parallelism == 0 || params.parallelism > Max_Parallelism) {
				throw "invalid # of parallelism";
			}

			[[unlikely]] if (params.memory_blocks < 8 * params.parallelism) {
				throw "invalid # of memory size";
			}

			// Should also check params.tag_length validity, but as we are not interested in final hash, we can omit that 
			// so RandomX algorithm may work (tag_length is 0).
		}
		...
		*/
		void makeFirstPass(Memory& memory, const std::array<std::byte, Initial_Hash_Size>& hash, const Params& params) {
			using namespace modernRX::blake2b;
			...
			for (uint32_t lane = 0; lane < params.parallelism; ++lane) {
				...
				// const auto ref_lane = lane; // for slice 0, ref_lane is always current lane
				...
				for (uint64_t prev_idx = 2, idx = 3; idx < blocks_per_slice; ++prev_idx, ++idx) {
					...
					const auto J1 = stdexp::value_cast<uint64_t>(memory[prev_idx]) & 0x00000000ffffffff;
			...
			for (uint32_t slice = 1; slice < Sync_Points; slice++) {
				for (uint32_t lane = 0; lane < params.parallelism; lane++) {
					...
					for (uint32_t idx = 0; idx < blocks_per_slice; idx++) {
						auto cur_idx = lane_start_idx + slice * blocks_per_slice + idx;
						auto prev_idx = cur_idx - 1;
						const auto J1 = stdexp::value_cast<uint64_t>(memory[prev_idx]) & 0x00000000ffffffff;
						const auto J2 = stdexp::value_cast<uint64_t>(memory[prev_idx]) >> 32;
				...
				for (uint32_t slice = 0; slice < Sync_Points; slice++) {
					for (uint32_t lane = 0; lane < params.parallelism; lane++) {
						...
						for (uint32_t idx = (slice == 0); idx < blocks_per_slice; idx++) {
							...
							shift_index = (shift_index + z) % blocks_per_lane; // this may overflow blocks_per_lane so have to be rolled over
