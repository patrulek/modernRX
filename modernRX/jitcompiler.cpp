...
namespace modernRX {
	namespace {
		[[nodiscard]] void emitAVX2Instruction(assembler::Context& asmb, const SuperscalarInstruction& instr, int32_t& data_offset) noexcept;
	}
	...
	jit_function_ptr<JITDatasetItemProgram> compile(const_span<SuperscalarProgram, Rx_Cache_Accesses> programs) {
		...
		for (uint32_t i = 0; i < 8; ++i) {
			...
			for (uint32_t j = 0; j < program.size; ++j) {
				...
				emitAVX2Instruction(asmb, instr, data_offset);
			}
	...
	namespace {
		...
		void emitAVX2Instruction(assembler::Context& asmb, const SuperscalarInstruction& instr, int32_t& data_offset) noexcept {
			using namespace assembler::registers;
			using namespace assembler;
