...
static auto key{ stdexp::span_cast<std::byte, test_key.size() - 1>(test_key) };
static auto input{ stdexp::span_cast<std::byte, test_input.size() - 1>(test_input) };
static auto input2{ stdexp::span_cast<std::byte, test_input2.size() - 1>(test_input2) };
...
void testBlake2bHash() {
	...
	auto expected = stdexp::byte_array(
	...
	blake2b::hash(hash, stdexp::span_cast<std::byte>(data), std::span<std::byte>());
	testAssert(hash == expected);

	auto data2 = stdexp::byte_array(
...
void testArgon2dBlake2bHash() {
	...
	testAssert(hash == std::vector<std::byte>{ expected.begin(), expected.end() });
}
